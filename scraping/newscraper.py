                            ...
                            for download_link_tag in download_link_tags:
                                if download_link_tag.text == '' and download_link_tag.text != 'PDF' and download_link_tag.text != 'Text':  
                                    ...
                                    try:
                                        ...
                                        try:
                                            log("         " + "ERROR")
                                            log("         " + "No table found at: " + str(root_url + "/fdsys/" + download_link_tag['href']))
                                            log("         " + "Attempting to sleep at re-retrieve table")
                                            time.sleep(1)
                                            table = more_soup.find('table', class_="page-details-budget-metadata-table")
                                            leaf_links = table.find_all('a')
                                        except:
                                            log("         " + "Failed to re-retrieve table at: " + str(root_url + "/fdsys/" + download_link_tag['href']))
                                            leaf_links = []
                                    text_file_name = ""
